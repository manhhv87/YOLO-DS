...
of YOLOv8m that routes the captured frame and the golden reference through a
...
${\sim}70\,$K parameters and is initialised so the network starts as the
YOLOv8m baseline, keeping the pretrained prior fully intact. DS-YOLO is
compared against four ablations---Diff-only, Stack6, and F-Sub---on an
in-house dataset of $75$ raw captures ($211$ after Roboflow augmentation)
from a single SMT board layout, covering four component types and
$5{,}420$ component-level OK/NG instances. DS-YOLO improves mAP@0.5 from
\TODO{$x_0$} to \TODO{$x_1$} ($+\TODO{$\Delta$}$~pp) over the RGB-only
baseline while incurring less than \TODO{$\delta t$}~ms additional latency.
