...
of YOLOv8m in which the captured frame and the golden reference are aligned
via an ORB homography, then routed through a shared backbone in parallel;
their multi-scale feature maps at $P_3, P_4, P_5$ are fused by a learnable
\emph{Cross-Reference Fusion Module} (CRFM) before the PANet neck and
detection head. The CRFM adds only ${\sim}70\,$K parameters and is
initialised so the network starts as the YOLOv8m baseline, keeping the
pretrained prior fully intact. We further evaluate \textbf{RG-YOLO}, a
lightweight early-fusion variant that appends the per-pixel absolute
difference $|\Ialn-\Igolden|$ as a fourth input channel, offering a
strong and deployment-friendly reference. Both designs are compared
against four ablations on an in-house dataset of $75$ raw captures
($211$ after Roboflow augmentation) from a single SMT board layout,
covering four component types and $5{,}420$ component-level OK/NG instances.
DS-YOLO improves mAP@0.5 from \TODO{$x_0$} to \TODO{$x_1$}
($+\TODO{$\Delta$}$~pp) over the RGB-only baseline while incurring less
than \TODO{$\delta t$}~ms additional latency. The detector is deployed on a
working SMT cell where a PC-side vision server returns OK/NG verdicts to a
Mitsubishi MELSEC-Q PLC via OPC~UA, and the PLC orchestrates two
articulated robots over CC-Link to physically sort defective boards.
\end{abstract}
...
\begin{itemize}
    ...
    detection} problem and systematically evaluate five ways to condition
    a YOLOv8m detector on the golden reference $\Igolden$: input-level
    concatenation (Stack6), difference-channel injection (Diff-only,
    RG-YOLO), raw feature subtraction (F-Sub) and the proposed learnable
    feature-level fusion (DS-YOLO).
    ...
    ($0.27\,\%$ of the backbone) and is initialised to the identity so
    ...
    CC-Link and OPC~UA, and we report end-to-end accuracy, latency and
...
\begin{description}
    ...
    parameters) and (b) early input-level fusion (RG-YOLO), because the
...
To isolate the contribution of each design choice we evaluate six
configurations on the same YOLOv8m backbone:
\begin{enumerate}
    \item \textbf{RGB} (baseline): single-stream YOLOv8m on $\Ialn$.
    \item \textbf{Diff-only}: single-stream YOLOv8m on $\Idiff$ only,
    discarding RGB appearance.
    ...
    channel carrying $|\Ialn\!-\!\Igolden|$ (early input-level fusion).
...
source in the corresponding split (yielding $147$ / $32$ / $32$ images
respectively). To probe H3 we additionally built three training subsets
containing $25\%$, $50\%$ and $100\%$ of the training images.
...
Split (by source): train / val / test      & $53$ / $11$ / $11$ sources ($147$ / $32$ / $32$ images)\\
...
All variants were trained for 60 epochs at input size $640\times640$
with batch size~4 on \TODO{GPU type, e.g.\ NVIDIA RTX 3090 24~GB} using
SGD with lr$_0$=0.01, momentum=0.937 and weight decay=$5\!\times\!10^{-4}$.
The RGB, Diff-only, Stack6 and RG-YOLO variants use the Ultralytics training
pipeline; DS-YOLO and F-Sub use a custom PyTorch loop (the dual-stream
forward pass requires manual golden-image injection). All dual-stream
variants are initialised from the public \texttt{yolov8m.pt} checkpoint:
backbone, neck and head weights are copied directly; the CRFM modules are
initialised with Kaiming weights for the convolutions and $\alpha_\ell=0$.
Augmentations were limited to mosaic, HSV jitter and horizontal flip;
strong geometric augmentations (random affine, scale, shear) were disabled,
as they would force invariance to the precise spatial alignment cue that
reference-guided detection relies on.
...
parameter overhead, supporting H1. The lightweight RG-YOLO already
recovers significant gain, confirming that reference conditioning is
useful even at the input level. F-Sub (raw feature subtraction with zero
extra parameters) performs \TODO{below/above} RG-YOLO, demonstrating that
naive feature-difference discards too much spatial context of the capture
stream. DS-YOLO with the learnable CRFM gate further improves over F-Sub
and RG-YOLO by $\TODO{$\Delta'$}$ pp, supporting H2.

\begin{table}[t]
\caption{Main results on the test split (YOLOv8m backbone, 60 epochs).
$\dagger$~trained via dual-stream custom loop; all others via Ultralytics.}
\label{tab:main}
\centering
\footnotesize
...
RG-YOLO (4-ch input)              & \TODO{} & \TODO{} & \TODO{} & \TODO{}\\
F-Sub (feat.\ subtraction)$\dagger$ & \TODO{} & \TODO{} & \TODO{} & \TODO{}\\
\textbf{DS-YOLO (ours)}$\dagger$  & \TODO{} & \TODO{} & \TODO{} & \TODO{}\\
\bottomrule
\end{tabular}}
\end{table}
...
and evaluated on the same test split. Fig.~\ref{fig:datafrac} shows
that the gap between DS-YOLO and the RGB baseline grows as the training
...
measured the resulting mAP. We evaluate the RGB baseline (unaffected by
alignment quality) and RG-YOLO (whose fourth-channel difference is directly
degraded by misalignment). Results are reported in Table~\ref{tab:robust}.
RG-YOLO maintains its advantage over RGB up to
$\sigma_t\!\approx\!\TODO{10}$~px and only drops below the baseline beyond
$\sigma_t\!\approx\!\TODO{15}$~px\,---\,well above the residual registration
error observed on the production cell ($\sigma_t\!\le\!\TODO{}$~px in
practice), confirming practical robustness.
...
RG-YOLO      & \TODO{} & \TODO{} & \TODO{} & \TODO{} & \TODO{}\\
\bottomrule
\end{tabular}}
\end{table}
...
is reported in Table~\ref{tab:latency}. We compare the RGB baseline,
RG-YOLO (one extra channel; difference computed on CPU) and DS-YOLO
(second backbone pass on the golden image). The dominant cost is the
YOLOv8m forward pass; the additional stages add \TODO{X}~ms for RG-YOLO
and \TODO{Y}~ms for DS-YOLO relative to the RGB baseline.
...
\resizebox{\columnwidth}{!}{\begin{tabular}{@{}l c c c@{}}
\toprule
\textbf{Stage} & \textbf{RGB} & \textbf{RG-YOLO} & \textbf{DS-YOLO}\\
\midrule
Capture (camera + transfer)      & \TODO{} & \TODO{} & \TODO{}\\
ORB + homography alignment       & \TODO{} & \TODO{} & \TODO{}\\
Diff channel computation         & ---     & \TODO{} & ---    \\
YOLOv8m forward (capture)        & \TODO{} & \TODO{} & \TODO{}\\
YOLOv8m forward (golden, shared) & ---     & ---     & \TODO{}\\
CRFM + neck + head               & ---     & ---     & \TODO{}\\
Decision + OPC UA write          & \TODO{} & \TODO{} & \TODO{}\\
\midrule
...
\begin{figure}[!ht]
    ...
    \caption{Detection comparison on the same board: YOLOv8m RGB baseline
